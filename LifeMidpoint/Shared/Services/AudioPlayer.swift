...
final class AudioPlayer: ObservableObject {
    ...
    /// - loop: true 时无限循环
    /// - loops: 指定额外重复次数 (numberOfLoops). 优先级高于 loop;
    ///   例如 loops = 2 表示总共播放 3 次 (脚步声需求). 不传则按 loop 处理.
    func play(file: String, channel: Channel, loop: Bool = false, loops: Int? = nil, volume: Float = 1.0) {
        ...
        do {
            ...
            player.numberOfLoops = loops ?? (loop ? -1 : 0)
