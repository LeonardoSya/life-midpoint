...
struct MedicationRecordView: View {
    ...
    var body: some View {
        MedicationShell(
            title: "服药提醒",
            trailing: "plus",
            trailingRoute: .editReminder,
            secondTrailing: "ellipsis",
            secondTrailingRoute: .myMedications,
            onBack: { dismiss() }
        ) {
            VStack(spacing: 22) {
                weekCard
        ...
        .overlay(alignment: .bottomTrailing) {
            NavigationLink(value: HealthDashboardView.HealthRoute.editReminder) {
                Image(systemName: "plus")
                    ...
                    .background(Color(hex: 0x7BC5E2), in: Circle())
                    .shadow(color: Color(hex: 0x7BC5E2).opacity(0.5), radius: 8, y: 4)
            }
            .buttonStyle(.plain)
            .padding(.trailing, 24)
            .padding(.bottom, 32)
        ...
        HStack(spacing: 12) {
            ...
            if done {
                Image(systemName: "checkmark")
                    ...
                    .background(Color(hex: 0x7BC5E2, alpha: 0.46), in: RoundedRectangle(cornerRadius: 10))
            } else {
                Circle()
                    .stroke(Color.black.opacity(0.3), style: StrokeStyle(lineWidth: 1.5, dash: [3, 3]))
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            VStack(spacing: 4) {
                ForEach(options, id: \.self) { option in
                    HStack { Circle().fill(Color(hex: 0x7BC5E2, alpha: 0.6)).frame(width: 32, height: 32); Text(option).font(AppFont.body(15)).foregroundStyle(Color.deepCharcoal).tracking(-0.15); Spacer(); Circle().stroke(Color(hex: 0x7BC5E2, alpha: 0.35), lineWidth: 2).frame(width: 16, height: 16) }
...
private struct MedicationShell<Content: View>: View {
    ...
    var secondTrailing: String? = nil
    var secondTrailingRoute: HealthDashboardView.HealthRoute? = nil
    let onBack: () -> Void
            ...
            .overlay(alignment: .top) {
                ZStack {
                    ...
                    HStack(spacing: 10) {
                        ...
                        Spacer()
                        trailingButton(trailing, route: trailingRoute)
                        if let secondTrailing {
                            trailingButton(secondTrailing, route: secondTrailingRoute)
                        }
                    }
    ...
    @ViewBuilder
    private func trailingButton(_ icon: String, route: HealthDashboardView.HealthRoute?) -> some View {
        if let route {
