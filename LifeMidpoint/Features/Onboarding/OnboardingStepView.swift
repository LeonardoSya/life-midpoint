...
struct OnboardingStepView: View {
    ...
    /// 由父流程控制: 仅当本页"完全加载完毕"(转场 / 渐变全部结束)后才为 true。
    /// 用于把对话气泡与分屏揭示推迟到页面完全呈现之后再开始, 默认 true 方便独立预览。
    ...
    @State private var splitBottomRevealed = false
    ...
    var body: some View {
        // 用 GeometryReader 锁定真实屏幕宽度: 背景图 scaledToFill 会让 ZStack 容器
        // 比可视屏幕更宽 (横向溢出), 若直接居中前景内容会整体右移。
        // 这里把前景 VStack 严格约束到 geo.size, 保证气泡左右 padding 对称居中。
        GeometryReader { geo in
            ZStack {
                backgroundImage

                // 第 7 页: 下半屏纯黑遮罩, 0.5s 后渐隐显出下半画面
                if step.revealStyle == .splitTopBottom {
                    VStack(spacing: 0) {
                        ...
                        Rectangle()
                            ...
                            .opacity(splitBottomRevealed ? 0 : 1)
                    }
    ...
    private func dialogueGroup(texts: [String]) -> some View {
        ...
        // 气泡组填满可用宽度, 由调用处的 .padding(.horizontal, 30) 形成左右对称留白,
        // 不再用 UIScreen 硬编码固定宽度 (会导致整组偏移而非居中).
    ...
    /// 页面完全呈现后才触发揭示(对话气泡 / 分屏), 且全程只触发一次。
    private func triggerRevealIfReady() {
        guard isActive, !hasStartedReveal else { return }
    ...
    private func startReveal() {
        if step.revealStyle == .splitTopBottom {
            // 第 7 页: 上半屏先显示 → 0.5s 后显出下半屏 → 再 0.5s 后才出文字
            splitBottomRevealed = false
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                withAnimation(.easeOut(duration: 0.4)) { splitBottomRevealed = true }
            }
            animateDialogues(startDelay: 1.0)
        } else {
            animateDialogues(startDelay: 0)
        }
    }

    private func animateDialogues(startDelay: Double) {
        visibleDialogues = []
        for (index, dialogue) in step.dialogues.enumerated() {
            DispatchQueue.main.asyncAfter(deadline: .now() + startDelay + Double(index) * 1.2) {
