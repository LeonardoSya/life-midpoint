...
enum StampLibrary {
    ...
    /// 按 `StampInfo.id` (即 `UserStamp.stampDefinitionId`) 查找邮票的图案/文案.
    static func info(for id: String) -> StampInfo? {
        allSeries.lazy.flatMap(\.stamps).first { $0.id == id }
    }
}
