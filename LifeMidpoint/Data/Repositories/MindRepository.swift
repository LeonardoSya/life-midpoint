...
struct MindRepository {
    ...
    @discardableResult
    func startMicroBehavior(templateId: String, ambient: String, didSwapTemplate: Bool = false) -> MicroBehaviorSession {
        let s = MicroBehaviorSession(templateId: templateId, ambientSoundKey: ambient, didSwapTemplate: didSwapTemplate)
        context.insert(s)
        try? context.save()
        return s
    }
