...
struct DiaryRepository {
    ...
    /// 查询某一天的日记总结记录 (用于"查看完整回忆"详情页).
    func entry(on date: Date) -> DiaryEntry? {
        let calendar = Calendar.current
        let start = calendar.startOfDay(for: date)
        guard let end = calendar.date(byAdding: .day, value: 1, to: start) else { return nil }
        let predicate = #Predicate<DiaryEntry> { $0.entryDate >= start && $0.entryDate < end }
        let d = FetchDescriptor<DiaryEntry>(predicate: predicate)
        return (try? context.fetch(d))?.first
    }
    ...
    /// 查询某一天记录的情绪打卡 (用于"查看完整回忆"详情页展示当日心情).
    func emotionLogs(on date: Date) -> [EmotionLog] {
        let calendar = Calendar.current
        let start = calendar.startOfDay(for: date)
        guard let end = calendar.date(byAdding: .day, value: 1, to: start) else { return [] }
        let predicate = #Predicate<EmotionLog> { $0.recordedAt >= start && $0.recordedAt < end }
        let d = FetchDescriptor<EmotionLog>(
            predicate: predicate,
            sortBy: [SortDescriptor(\.recordedAt, order: .reverse)]
        )
        return (try? context.fetch(d)) ?? []
...
// MARK: - DailyEmotionProviding

extension DiaryRepository: DailyEmotionProviding {}
