...
HOST_FILE="$LOG_DIR/agent-server.host"
ENV_PATH="${AGENT_ENV_PATH:-$SERVER_DIR/.env}"

mkdir -p "$LOG_DIR"

# 已有进程存活, 且监听地址与本次期望一致 (比如都是 127.0.0.1, 或都是 0.0.0.0) 才能直接复用;
# 否则 (例如上次是模拟器用的 127.0.0.1, 这次是真机调试要的 0.0.0.0) 必须重启才能改变监听范围。
RUNNING_HOST="$(cat "$HOST_FILE" 2>/dev/null || true)"
if [[ "$RUNNING_HOST" == "$HOST" ]] && /usr/bin/curl -fsS --max-time 1 "$HEALTH_URL" >/dev/null 2>&1; then
  echo "[AgentServer] Already running at $HEALTH_URL (host=$HOST)"
  exit 0
fi
...
if [[ -f "$PID_FILE" ]]; then
  ...
  if [[ -n "$OLD_PID" ]] && kill -0 "$OLD_PID" >/dev/null 2>&1; then
    if [[ "$RUNNING_HOST" == "$HOST" ]]; then
      ...
      echo "[AgentServer] Existing process bound to host=${RUNNING_HOST:-unknown}, need host=$HOST; restarting."
    fi
...
echo "[AgentServer] Starting at $HEALTH_URL (host=$HOST)"
echo "[AgentServer] Log: $LOG_FILE"

(
  cd "$SERVER_DIR"
  AGENT_PORT="$PORT" AGENT_HOST="$HOST" AGENT_ENV_PATH="$ENV_PATH" nohup bun src/server.ts >>"$LOG_FILE" 2>&1 &
  echo $! >"$PID_FILE"
)
echo "$HOST" >"$HOST_FILE"
