...
/**
 * 单个角色的上下文: 只保留用户发言与"这个角色自己"过去的回复, 且不加任何角色名前缀。
 * 这样模型看到的自己的历史发言都是干净的第一人称, 从根上避免它用"奶奶""小女孩"自称。
 */
function personaContext(context: SessionMessage[], persona: PersonaId): ModelMessage[] {
  return context
    .filter((message) => message.speaker === 'user' || message.speaker === persona)
    .map((message) => ({
...
/**
 * 总结用的上下文: 保留"用户/奶奶/小女孩"标签, 让整理 agent 能分辨三方对话。
 */
function labeledContext(context: SessionMessage[]): ModelMessage[] {
